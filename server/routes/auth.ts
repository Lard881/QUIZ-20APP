...
export const updateProfile: RequestHandler = (req, res) => {
  try {
    ...
    if (!instructor) {
      return res
        .status(404)
        .json({ error: "NOT_FOUND", message: "Instructor not found" });
    }

    const { name, email, currentPassword, newPassword } =
      req.body as UpdateInstructorRequest;
    ...
    if (email) {
      const emailExists = instructors.find(
        (i) =>
          i.id !== instructorId &&
    ...
    if (newPassword && currentPassword) {
      const isCurrentPasswordValid =
        ...
        (instructor.password &&
          verifyPassword(currentPassword, instructor.password));
    ...
    res
      .status(500)
      .json({ error: "UPDATE_FAILED", message: "Profile update failed" });
  }
};
...
export const getProfile: RequestHandler = (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "FETCH_FAILED", message: "Failed to get profile" });
  }
};
